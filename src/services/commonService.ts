...
export class CommonService {
  async getOptions(data: any, language?: string): Promise<any> {
    // TODO 传多个value查多个
    let result = [];
    try {
      if (data.type === 'category') {
        ...
        result = res.data.data.map(item => ({
          ...
          enLabel: item.categoryNameEn
        }));
      } else if (data.type === 'provider') {
        const res = await Api.get<AxiosResponse<OsiDbProviderSchema.ListAll[]>>(
          `/api/outsourcing/OsiDbProvider/listAll`
        );
        result = res.data.data.map(item => ({
          value: item.id + '',
          label: item.name,
  ...
  async getImageAllReason(data?: any): Promise<any> {
    const res = await Api.get(`/api/outsourcing/reason`);
